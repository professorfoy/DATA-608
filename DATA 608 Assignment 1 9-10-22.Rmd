...
```{r fig.height = 3, fig.width = 1.25, fig.align = "left"}
# By default, most of the data visualization tools will order the categorical groups alphabetically. The base R function reorder(State,n) will effectively reorder the State values based on the n column. The bar chart is flipped using the coord_flip function and since it is flipped, the labels are inversed. The graph was resized to allow it to fit without bunching State abbreviation names. The label for the graph was resized to fit using the theme(plot.title) function. 
ggplot(data = inc1, aes(x = reorder(State,n), y = n)) + 
  geom_bar(stat = "identity") +
  coord_flip() +
  theme(plot.title = element_text(size=6)) +
  labs(title = "States Ranked High to Low Distribution of Companies ",
       x = "State",
       y = "Number of Companies")
```
